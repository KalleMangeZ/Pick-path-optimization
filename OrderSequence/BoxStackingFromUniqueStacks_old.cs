...
public class BoxStackingFromUniqueOrderStacks_old
{
    Graph g;
    ...
    private void RepresentConfigurations() 
                ...
                if (requiredLayer1.All(x => layerSet.Contains(x)))
                {
                    foreach(var layer2 in configurations[i].Layers){
                        if(requiredLayer2.All(x => layer2.Boxes.Contains(x))){
                            Console.WriteLine(
                                $"\nMATCH FOUND → Layer: {string.Join(",", layer.Boxes)} " +  //if contains required order stacks in a layer
        ...
        if(matchNotFound){
            ...
            if(g.nbrOrdersPerLayers > uniqueOrderStacks.Count){
                ...
                CreateConfigurationsFromUniqueStacks_old finishBuildingConfig = new CreateConfigurationsFromUniqueStacks_old(g, uniqueOrderStacks);
            }
        }
                      
        CreateConfigurationsFromUniqueStacks_old finishBuildingConfig1 = new CreateConfigurationsFromUniqueStacks_old(g, uniqueOrderStacks);
    ...
    config contains required layers
    */
    ...
    HashSet<int> instanceOfOrders = new HashSet<int>();
    foreach(BoxLayerCombination layer in config.Layers){
        foreach(int order in layer.Boxes){
            if(instanceOfOrders.Contains(order)){
                Console.WriteLine("Duplicate order found in layer: " + order);
            } else {
                instanceOfOrders.Add(order);
            }
        }
    }
